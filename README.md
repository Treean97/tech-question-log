...
1. 플랫폼을 부모로 설정하기
 ...
 - 단점 :
    - 회전, 스케일까지 물려받아 의도치 않은 변형이 발생할 수 있음
    - 네트워크 동기화가 복잡해짐
...
3. 물리 조인트 방식
 - 장점 :
    - 물리 엔진이 붙어있는 효과를 자연스럽게 처리
    - 마찰, 힘 전달 등 복잡한 순동작까지 물리적으로 구현 가능

 - 단점 :
    - Joint 생성/파괴 비용, 파라미터 튜닝 필요
    - 캐릭터 컨트롤러와 호환되지 않음

4. 플랫폼 델타 추적 방식
 - 장점 :
    - 부모 자식계층을 변경하지 않으므로 1번에 대한 단점이 없음
    - 캐릭터 컨트롤러, 네브 매쉬 에이전트 등 어디든 적용 가능
