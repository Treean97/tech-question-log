...
## Q : NavMesh를 통해 런타임 중 동적으로 길 연결 및 차단

차단은 NavMeshObstacle로 가능하고, 거꾸로 Obstacle을 해제하면 다시 연결된다.

하지만 런타임 중 생성되는 연결용 객체(A 지점과 B 지점을 이어주는 길 오브젝트)는
방법 1. NavMeshLink를 사용
  - 장점 : 가벼움
  - 단점 : 자연스러운 연결을 위해 조절이 필요함
