...
SetParent의 파라미터 중 worldPositionStays는 부모 변경 시 자식 오브젝트가 보존할 값을 정한다
 - true 라면 부모를 바꿔도 월드 좌표를 유지한다
 - false 라면 부모를 바꿀 때 로컬 좌표 유지한다


## 걷거나 매달리는 애니메이션 벽에 자연스럽게 붙이는 법 (Ray, IK, Rigging)

ray를 통해 접촉점을 사전에 판단하고 IK 타겟을 보간하여 손이나 발의 위치가 자연스럽게 이동하도록 한다.


## 오브젝트 파괴 연출

1. 사전에 미리 분할된 프리팹을 사용
 - 장점 :
    - 런타임 비용이 거의 없음
    - 의도한 모양대로 연출 가능
  
 - 단점 :
    - 조각 수가 많아지면 메모리 관리 부담
    - 동적으로 새로운 형태를 만들기 어려움
 
2. 런타임 중 메쉬 슬라이싱
 - 장점 :
    - 파괴 지점, 범위를 자유롭게 지정 가능
    - 조각 형태를 동적으로 생성 가능
